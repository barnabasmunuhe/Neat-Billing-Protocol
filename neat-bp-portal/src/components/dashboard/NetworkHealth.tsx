...
import { CheckCircle2, Cpu, Wifi, AlertTriangle, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-white/[0.08] bg-[#0A0D16] p-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-medium text-zinc-300">Overall Network Status: <span className="text-emerald-400">Excellent</span></p>
          <p className="mt-1 text-xs text-zinc-500">All core systems are operational and performing well.</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-6">
        ...
        <button className="group flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-2.5 text-sm font-medium text-zinc-300 transition-all hover:border-purple-500/30 hover:bg-purple-500/10 hover:text-white">
          View Network Health
          <ArrowRight size={16} className="text-zinc-500 transition-transform group-hover:translate-x-0.5 group-hover:text-purple-400" />
        </button>
      </div>
    </div>
  );
